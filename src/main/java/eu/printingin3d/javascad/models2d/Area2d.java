...
public class Area2d extends AbstractCollection<Coords2d> {
    ...
    /**
     * Moves this list of coordinates with the given coordinate. Creates a new
     * object, this object remains untouched.
     * 
     * @param move
     *            the coordinate move with
    ...
    /**
     * Rotates this list of coordinates with the given degree. Creates a new
     * object, this object remains untouched.
     * 
     * @param angle
     *            the angle in degree to rotate with
     * @return a new object with the new coordinates
     */
    public Area2d rotate(Angle angle) {
        List<Coords2d> result = new ArrayList<>();
        for (Coords2d c : coords) {
            result.add(new Coords2d(angle.cos() * c.getX() - angle.sin()
                    * c.getY(), angle.sin() * c.getX() + angle.cos() * c.getY()));
        }
     ...
     * 
     * @param z
     *            the Z value to be used for the transformation
    ...
    /**
     * Iterates over a list of 2D coordinates. Used internally by the union
     * function.
     */
    private static class Area2dIterator {
        ...
        public Coords2d skipTillOutside(Area2d c1) {
            LineSegment2d curr = LineSegment2d.startLineSegmentSeries(coords
                    .get(current));
            for (int i = coords.size(); i > 0; i--) {
                Coords2d c = next();
                curr = curr.next(c);
                if (c1.calculatePointRelation(c) != PointRelation.INSIDE) {
                    return findClosest(c1.findCrossing(curr, true),
                            curr.getStart());
                }
            }
            throw new IllegalValueException(
                    "There is no point outside of the given area in this area.");
        }
    }
    ...
    private Pair<Coords2d, Area2dIterator> getIteratorCrossing(
            final LineSegment2d segment) {
        Set<Pair<Coords2d, Area2dIterator>> closest = new TreeSet<>(
                new Comparator<Pair<Coords2d, Area2dIterator>>() {
                    @Override
        ...
        for (LineSegment2d current : LineSegment2d.lineSegmentSeries2d(coords)) {
            if (current.hasCommon(segment)) {
                closest.add(new Pair<Coords2d, Area2dIterator>(
                        current.getEnd(), new Area2dIterator(i, coords)));
            } else {
                Coords2d cross = EdgeCrossSolver.findCross(segment, current);
                if (cross != null) {
                    closest.add(new Pair<Coords2d, Area2dIterator>(cross,
                            new Area2dIterator(i, coords)));
                }
            }
            i++;
        }
        if (closest.isEmpty()) {
            throw new RuntimeException("There is no cross for " + segment
                    + " in this area.");
        }
    ...
    public Area2d union(Area2d other) {
        AssertValue.isFalse(isDistinct(other),
                "Cannot create a union of two distinct areas!");
        ...
        List<Coords2d> result = new ArrayList<>();
        Pair<Area2dIterator, Area2dIterator> iterators = new Pair<>(
                getIteratorOutside(other), null);
        ...
        List<Coords2d> crosses = areas.getValue2().findCrossing(
                new LineSegment2d(c, prev), false);
        if (!crosses.isEmpty()) {
        ...
        while (iterators.getValue1().hasNext()
                || iterators.getValue2().hasNext()) {
            ...
            if (!crosses.isEmpty()) {
                ...
                if (iterators.getValue2() == null) {
                    Pair<Coords2d, Area2dIterator> pair = areas.getValue2()
                            .getIteratorCrossing(current);
                    iterators = new Pair<>(pair.getValue2(),
                            iterators.getValue1());
                } else {
                ...
                if (iterators.getValue2() == null) {
                    ...
                    cross = iterators.getValue2().skipTillOutside(
                            areas.getValue1());
                    iterators = iterators.reverse();
                }
                if (cross != null) {
     ...
     *            if the cross point is one of the two given end points this
     *            method will return null if this parameter is false
     * @return true if and only if the given line segment crosses any line
     *         segment of area.
     */
    public List<Coords2d> findCrossing(LineSegment2d segment,
        ...
        for (LineSegment2d current : LineSegment2d.lineSegmentSeries2d(coords)) {
            if (current.hasCommon(segment)) {
                ...
                if (common != null) {
                    ...
                    Coords2d cross = EdgeCrossSolver
                            .findCross(segment, current);
                    if (cross != null
                            && (includeEndPoints || !segment.contains(cross))) {
                        result.add(cross);
                    }
                }
            }
     ...
     *         returns with <b>BORDER</b> if the point is on the border of this
     *         area.
     */
    public PointRelation calculatePointRelation(Coords2d p) {
        LineSegment2d horizontal = new LineSegment2d(new Coords2d(-1E10,
                p.getY()), new Coords2d(+1E10, p.getY()));

        LineSegment2d current = new LineSegment2d(null, coords.get(coords
                .size() - 1));
            ...
            if (common != null) {
                ...
                if (cross != null) {
                    if (cross.equals(current.getStart())) {
                        Coords2d prevv = coords.get(Math.floorMod(i - 2,
                                coords.size()));
                        if (EdgeCrossSolver.findCross(horizontal,
                                new LineSegment2d(c, prevv)) != null) {
                            crosses.add(cross);
                        }

                    } else if (cross.equals(c)) {
                        Coords2d next = coords.get(Math.floorMod(i + 1,
                                coords.size()));
                        if (EdgeCrossSolver.findCross(horizontal,
                                new LineSegment2d(next, current.getStart())) != null) {
                            crosses.add(cross);
                        }
                    } else {
                        crosses.add(cross);
     ...
     * 
     * @param index
     *            the index of the point we want to get.
    ...
    /**
     * Returns with a list of coordinates which contains the coordinates indexed
     * from start to end inclusive. The end index can be smaller then start
     * index in which case the values are start over after the last item and
     * start from zero.
     * 
     * @param start
    ...
    public Area2d subList(int start, int end) {
        if (end < start) {
            List<Coords2d> result = new ArrayList<>(coords.subList(start,
                    coords.size()));
    ...
    private int findCoords(Coords2d c) {
        ...
        throw new IllegalValueException(
                "The given value is not part of this list: " + c);
    }

    /**
     * Returns with a list of coordinates which contains the coordinates from
     * <b>first</b> to <b>last</b> inclusive. The last item can be before then
     * first in which case the values are start over after the last item and
     * start from first.
     * 
     * @param first
    ...
    /*
     * private static boolean isInsideTriangle(Coords2d a, Coords2d b, Coords2d
     * c, Coords2d p) { // Compute vectors Coords2d v0 = new
     * Coords2d(c.getX()-a.getX(), c.getY()-a.getY()); Coords2d v1 = new
     * Coords2d(b.getX()-a.getX(), b.getY()-a.getY()); Coords2d v2 = new
     * Coords2d(p.getX()-a.getX(), p.getY()-a.getY());
     * 
     * // Compute dot products double dot00 = v0.dot(v0); double dot01 =
     * v0.dot(v1); double dot02 = v0.dot(v2); double dot11 = v1.dot(v1); double
     * dot12 = v1.dot(v2);
     * 
     * // Compute barycentric coordinates double invDenom = 1.0 / (dot00 * dot11
     * - dot01 * dot01); double u = (dot11 * dot02 - dot01 * dot12) * invDenom;
     * double v = (dot00 * dot12 - dot01 * dot02) * invDenom;
     * 
     * // Check if point is in triangle return u>=0 && v>=0 && u + v<=1; }
     */
