...
/**
 * Mirrors a model. The plane of the mirroring could only be the X, Y and Z
 * plane, to make it easier to use and understand.
 *
...
public final class Mirror extends Complex3dModel {
     ...
     * 
     * @param model
     *            the model to be mirrored
     * @return the mirrored model
     * @throws IllegalValueException
     *             if the model is null
     */
    public static Mirror mirrorX(Abstract3dModel model)
            throws IllegalValueException {
     ...
     *             if the model is null
     */
    public static Mirror mirrorY(Abstract3dModel model)
            throws IllegalValueException {
     ...
     *             if the model is null
     */
    public static Mirror mirrorZ(Abstract3dModel model)
            throws IllegalValueException {
    ...
    private Mirror(Abstract3dModel model, Direction direction)
            throws IllegalValueException {
        AssertValue.isNotNull(model,
                "The model to be mirrored must not be null!");
    ...
    protected SCAD innerToScad(IColorGenerationContext context) {
        return new SCAD("mirror(" + direction.getCoords() + ")").append(model
                .toScad(context));
    }
    ...
    protected Boundaries3d getModelBoundaries() {
        ...
        return new Boundaries3d(boundaries.getX().negate(
                direction == Direction.X), boundaries.getY().negate(
                direction == Direction.Y), boundaries.getZ().negate(
