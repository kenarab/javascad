...
public class Difference extends Complex3dModel {
    ...
    /**
     * <p>
     * Creates the object with the models given. The first parameter will be the
     * model and the second list of models will be subtracted by it.
     * </p>
     * <p>
     * If the first parameter is null it throws an IllegalValueException. If the
     * second parameter is null or empty the operation do nothing, just gives
     * back the first model.
     * </p>
     * 
     * @param model1
     *            the model subtracted from
     * @param model2
     *            the model to be subtracted
     * @throws IllegalValueException
     *             if the first model is null
     */
    public Difference(Abstract3dModel model1, List<Abstract3dModel> model2)
            throws IllegalValueException {
        AssertValue
                .isNotNull(model1,
                        "The first parameter of the difference operation should not be null!");

        this.model1 = model1;
        this.model2 = model2 == null ? Collections
                .<Abstract3dModel> emptyList() : ListUtils.removeNulls(model2);
    }

    /**
     * <p>
     * Creates the object with the list of models given. The first parameter is
     * used as a model and all subsequent models will be subtracted from it.
     * </p>
     * <p>
     * If the first parameter is null it throws an IllegalValueException. If any
     * subsequent parameter is null that parameter is ignored. If only one
     * parameter is given this operation does nothing, just gives back the first
     * model.
     * </p>
     * 
     * @param model1
     ...
     *             if the first model is null
     */
    public Difference(Abstract3dModel model1, Abstract3dModel... model2)
            throws IllegalValueException {
    ...
    protected Boundaries3d getModelBoundaries() {
        ...
        for (Abstract3dModel model : model2) {
            if (model instanceof Cube && !model.isRotated()) {
                ...
                } else if (y.isInsideOf(b.getY()) && z.isInsideOf(b.getZ())) {
                    x = x.remove(b.getX());
                }
            }
        }
