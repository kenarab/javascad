...
/**
 * Colors the object it contains. Please pay attention: colors are only
 * supported in the CSG view in OpenSCAD - neither CGAL rendering nor STL output
 * does not support it yet. This is a limitation of the OpenSCAD rendering.
 *
...
public class Colorize extends Complex3dModel {
     ...
     * 
     * @param color
     *            the color which will be used
     * @param model
     *            the model which will be colored
     * @throws eu.printingin3d.javascad.exceptions.IllegalValueException
     *             if either the model or the color null
     */
    public Colorize(Color color, Abstract3dModel model) {
        AssertValue
                .isNotNull(model, "The model shouldn't be null for colorize");
        AssertValue
                .isNotNull(color, "The color shouldn't be null for colorize");
    ...
    protected SCAD innerToScad(IColorGenerationContext context) {
        return baseModel.toScad(context)
                .prepend(getStringRepresentation(color));
    }
    ...
    public static String getStringRepresentation(Color color) {
        ...
        sb.append('[').append(DoubleUtils.formatDouble(color.getRed() / 255.0))
                .append(',')
                .append(DoubleUtils.formatDouble(color.getGreen() / 255.0))
                .append(',')
                .append(DoubleUtils.formatDouble(color.getBlue() / 255.0));

        if (color.getAlpha() < 255) {
            sb.append(',').append(
                    DoubleUtils.formatDouble(color.getAlpha() / 255.0));
        }
        sb.append(']');
