...
public final class TransformationFactory {
    ...
    public static ITransformation getTranlationMatrix(Abstract3d delta) {
        double[] elemenents = { 1, 0, 0, delta.getX(), 0, 1, 0, delta.getY(),
                0, 0, 1, delta.getZ() };
    ...
    public static ITransformation getRotationMatrix(Angles3d angles) {
        ...
        double[] elemenents = { x.getX(), y.getX(), z.getX(), 0, x.getY(),
                y.getY(), z.getY(), 0, x.getZ(), y.getZ(), z.getZ(), 0, };
        return new TransformationMatrix(elemenents);
    }
     ...
     * 
     * @param values
     *            the vector to be used for the scaling
    ...
    public static ITransformation getScaleMatrix(double x, double y, double z) {
        double[] elemenents = { x, 0, 0, 0, 0, y, 0, 0, 0, 0, z, 0 };
        return new TransformationMatrix(elemenents);
    }

    /**
     * Returns with a transformation representing a mirror on the given
     * direction.
     * 
     * @param direction
    ...
    public static ITransformation getMirrorMatrix(Direction direction) {
        return getScaleMatrix(direction.getCoords().mul(-2.0)
                .add(new Coords3d(1, 1, 1)));
    }
