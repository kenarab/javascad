 ...
 * This is enough to represent any 3D transformation and reduce the necessary
 * calculation a lot.
 * 
...
public class TransformationMatrix implements ITransformation {
    ...
    private static int getValuePosition(int row, int column) {
        AssertValue.isTrue(row >= 0 && row < 3,
                "Row number should be between 0 and 2, but was " + row);
        AssertValue.isTrue(column >= 0 && column < 4,
                "Column number should be between 0 and 3, but was " + column);

        return column + row * 4;
    }

    protected TransformationMatrix(double[] matrix) {
        AssertValue.isTrue(matrix.length == 12,
                "The representation of the matrix should contain 12 values!");
    ...
    public Coords3d transform(Coords3d vec) {
        double x = getValue(0, 0) * vec.getX() + getValue(0, 1) * vec.getY()
                + getValue(0, 2) * vec.getZ() + getValue(0, 3);
        double y = getValue(1, 0) * vec.getX() + getValue(1, 1) * vec.getY()
                + getValue(1, 2) * vec.getZ() + getValue(1, 3);
        double z = getValue(2, 0) * vec.getX() + getValue(2, 1) * vec.getY()
                + getValue(2, 2) * vec.getZ() + getValue(2, 3);
