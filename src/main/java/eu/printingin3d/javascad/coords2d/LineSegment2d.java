...
public class LineSegment2d extends LineSegment<Coords2d> {
    ...
    public static List<LineSegment2d> lineSegmentSeries2d(List<Coords2d> coords) {
        ...
        LineSegment2d current = startLineSegmentSeries(coords
                .get(coords.size() - 1));
    ...
    public boolean isParallel(LineSegment2d ls) {
        return DoubleUtils
                .equalsEps(
                        (start.getY() - end.getY())
                                * (ls.start.getX() - ls.end.getX()),
                        (ls.start.getY() - ls.end.getY())
                                * (start.getX() - end.getX()));
    }
    ...
    public boolean isOnLineSegment(Coords2d c) {
        if (DoubleUtils.between(c.getX(), start.getX(), end.getX())
            ...
            double left = (end.getY() - start.getY())
                    * (c.getX() - start.getX());
            double right = (end.getX() - start.getX())
                    * (c.getY() - start.getY());
     ...
     * Returns null if the two line segments are distinct.
     * 
     * @param lc
     *            the other line segment
    ...
    public LineSegment2d common(LineSegment2d lc) {
        ...
        return lcSide == null || thisSide == null || lcSide.equals(thisSide) ? null
                : new LineSegment2d(lcSide, thisSide);
    }
}
