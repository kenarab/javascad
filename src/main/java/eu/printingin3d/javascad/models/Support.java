...
/**
 * <p>
 * Represents support material to hold the above objects. This helps when the
 * object cannot be printed without support, but the slicer software cannot
 * handle support material.
 * </p>
 * <p>
...
public class Support extends Atomic3dModel {
    ...
    /**
     * Creates a support object. The resulting object is a <i>thickness</i> thin
     * continuous line which fills the given <i>dims</i> block.
     * 
     * @param dims
     *            the dimensions of the support object
     * @param thickness
     *            the thickness of the wall to be used when creating the object
     */
    ...
    private Abstract3dModel getModel() {
        ...
        if (dims.getX() > dims.getY()) {
            long sliceNum = Math.round((dims.getX() - thickness) / thickness
                    / SLICE_MUL);
            ...
            long sliceNum = Math.round((dims.getY() - thickness) / thickness
                    / SLICE_MUL);
        ...
        while (x < dims.getX() && y < dims.getY()) {
            slices.add(new Cube(sliceSize).align(
                    new Side(AlignType.MIN_IN, AlignType.MIN_IN,
                            AlignType.CENTER), base).move(
                    new Coords3d(x, y, 0.0)));
    ...
    protected Boundaries3d getModelBoundaries() {
        ...
        return new Boundaries3d(new Boundary(-x, x), new Boundary(-y, y),
                new Boundary(-z, z));
    }
