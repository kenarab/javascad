...
/**
 * Represents a cylinder, a truncated cone or a cone. It is a descendant of
...
public class Cylinder extends Atomic3dModel {
    ...
    /**
     * Creates a truncated cone. If one of the two radiuses is zero the result
     * is a cone. If the two radiuses are the same the result is a cylinder.
     * 
     * @param length
     ...
     *             if the length or any any of the two radius parameter is
     *             negative
     */
    public Cylinder(double length, Radius bottomRadius, Radius topRadius)
            throws IllegalValueException {
        super();
        isNotNegative(length, "The length should be positive, but " + length);
        isNotNull(bottomRadius,
                "Both radius should be given, but bottomRadius was null");
        isNotNull(topRadius,
                "Both radius should be given, but topRadius was null");
     ...
     * @throws IllegalValueException
     *             if the length or the radius parameter is negative
     */
     ...
     */
    @Deprecated
    public Cylinder(double length, double bottomRadius, double topRadius)
            throws IllegalValueException {
        this(length, Radius.fromRadius(bottomRadius), Radius
                .fromRadius(topRadius));
    }
    ...
    protected SCAD innerToScad(IColorGenerationContext context) {
        ...
        if (bottomRadius.equals(topRadius)) {
            return new SCAD("cylinder(h=" + DoubleUtils.formatDouble(length)
                    + ", r=" + bottomRadius + commandResolution
                    + ", center=true);\n");
        }
        return new SCAD("cylinder(h=" + DoubleUtils.formatDouble(length)
                + ", r1=" + bottomRadius + ", r2=" + topRadius
                + commandResolution + ", center=true);\n");
    }
    ...
    protected Boundaries3d getModelBoundaries() {
        ...
        double z = length / 2.0;
        return new Boundaries3d(Boundary.createSymmetricBoundary(r),
    ...
    protected CSG toInnerCSG(FacetGenerationContext context) {
        ...
        int numSlices = context.calculateNumberOfSlices(topRadius
                .min(bottomRadius));
        for (int i = 0; i < numSlices; i++) {
            double t0 = i / (double) numSlices;
            double t1 = (i + 1) / (double) numSlices;
            polygons.add(Polygon.fromPolygons(Arrays.asList(startV,
                    cylPoint(+z, topRadius, t1), cylPoint(+z, topRadius, t0)),
                    context.getColor()));
            polygons.add(Polygon.fromPolygons(
                    Arrays.asList(cylPoint(+z, topRadius, t0),
                            cylPoint(+z, topRadius, t1),
                            cylPoint(-z, bottomRadius, t1),
                            cylPoint(-z, bottomRadius, t0)

                    ), context.getColor()));
            polygons.add(Polygon.fromPolygons(Arrays.asList(endV,
                    cylPoint(-z, bottomRadius, t0),
                    cylPoint(-z, bottomRadius, t1)), context.getColor())
