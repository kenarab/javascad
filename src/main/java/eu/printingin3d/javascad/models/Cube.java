...
public class Cube extends Atomic3dModel {
     ...
     * @param maxCorner
     *            the corner with the higher coordinates
     * @return a cuboid with the given corners.
     * @throws eu.printingin3d.javascad.exceptions.IllegalValueException
     *             the minCorner has bigger value then maxCorner in any
     *             coordinate (x, y or z)
     */
    public static Abstract3dModel fromCoordinates(Coords3d minCorner,
            Coords3d maxCorner) {
        Abstract3dModel result = new Cube(new Dims3d(maxCorner.getX()
                - minCorner.getX(), maxCorner.getY() - minCorner.getY(),
                maxCorner.getZ() - minCorner.getZ()));
        return result.align(new Side(AlignType.MIN_IN, AlignType.MIN_IN,
                AlignType.MIN_IN), minCorner);
    }
     ...
     * 
     * @param size
     *            the 3D size value used to construct the cuboid
     */
     ...
     * 
     * @param size
     *            the size used to construct the cube
     */
    ...
    protected CSG toInnerCSG(FacetGenerationContext context) {
        ...
        for (int[] info : a) {
            ...
            for (int i : info) {
                Coords3d pos = new Coords3d(size.getX()
                        * (1 * Math.min(1, i & 1) - 0.5), size.getY()
                        * (1 * Math.min(1, i & 2) - 0.5), size.getZ()
