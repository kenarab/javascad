...
/**
 * An IFileExporter implementation for the textual STL file format. The usual
 * extension of the file is .stl. The exported file does not contain the color
...
public class StlTextFile implements IFileExporter {
    ...
    private static String facetToStlString(Facet facet) {
        StringBuilder sb = new StringBuilder().append("  facet normal ")
                .append(facet.getNormal().format(OutputFormat.STL))
                .append('\n').append("    outer loop\n");

        for (Vertex v : facet.getVertexes()) {
            sb.append("    vertex ").append(v.format(OutputFormat.STL))
                    .append('\n');
        }
