...
/**
 * Denotes the 3D angles - 3 number which denotes the angles on the X, Y and Z
 * planes This class is used for the rotation operations. The angle on each
 * plane will be normalized, which means the result will be always between -180
 * and 180 degrees.
 *
...
public class Angles3d extends Basic3dFunc<Angles3d> {
     ...
     * @param x
     *            the angle in degree
     * @return the Angles3d object represents x degree rotation around the X
     *         axis
     */
     ...
     * @param y
     *            the angle in degree
     * @return the Angles3d object represents y degree rotation around the Y
     *         axis
     */
     ...
     * @param z
     *            the angle in degree
     * @return the Angles3d object represents z degree rotation around the Z
     *         axis
     */
     ...
     * 
     * @param x
     *            the angle on the X plane
     * @param y
     *            the angle on the Y plane
     * @param z
     *            the angle on the Z plane
     */
    public Angles3d(double x, double y, double z) {
        super(normalize(x), normalize(y), normalize(z));
    }

    private static double simplicity(Coords3d temp, Normal3d normalVectorA,
            Normal3d normalVectorB) {
        double result = 0.0;
        ...
        if (!DoubleUtils.isZero(a)) {
            result = (a < 0.0 ? -1.0 : 1.0)
                    * Math.toDegrees(Math.acos(b / Math.sqrt(b * b + a * a)));
        }
        return result;
    }
    ...
    static {
        List<Coords3d> coords = Arrays.asList(Coords3d.X, Coords3d.Y,
                Coords3d.Z);
            ...
            for (Coords3d c2 : coords) {
                for (Coords3d c3 : coords) {
                    for (List<Normal3d> norms : Arrays.asList(
                        ...
                        results.add(new Config(c1, c2, c3, norms.get(0), norms
                                .get(1), norms.get(2)));
                    }
                }
            }
    ...
    /**
     * Rotates the current value by the given value - this object won't change,
    ...
    public Angles3d rotate(Angles3d delta) {
        for (Config conf : CONFIGS) {
            ...
            temp = conf.coord3.rotate(this).rotate(delta).rotate(zOnly(-gamma))
                    .rotate(yOnly(-beta));
            double alpha = simplicity(temp, conf.norm3, conf.norm1);

            if (Coords3d.xOnly(1.0).rotate(this).rotate(delta)
                    .rotate(zOnly(-gamma)).rotate(yOnly(-beta))
                    .rotate(xOnly(-alpha)).equals(Coords3d.xOnly(-1.0))) {
                alpha = -(180.0 + alpha);
                beta = -beta;
                gamma = 180.0 + gamma;
            }
            if (Coords3d.yOnly(1.0).rotate(this).rotate(delta)
                    .rotate(zOnly(-gamma)).rotate(yOnly(-beta))
                    .rotate(xOnly(-alpha)).equals(Coords3d.yOnly(-1.0))) {
                alpha = -(180.0 + alpha);
            }

            Angles3d result = new Angles3d(alpha, beta, gamma);

            if (Abstract3d.closeEquals(Coords3d.X.rotate(this).rotate(delta),
                    Coords3d.X.rotate(result))
                    && Abstract3d.closeEquals(
                            Coords3d.Y.rotate(this).rotate(delta),
                            Coords3d.Y.rotate(result))
                    && Abstract3d.closeEquals(
                            Coords3d.Z.rotate(this).rotate(delta),
                            Coords3d.Z.rotate(result))) {
                return result;
            }
        }

        throw new IllegalValueException("No solution found for adding " + this
                + " to " + delta
