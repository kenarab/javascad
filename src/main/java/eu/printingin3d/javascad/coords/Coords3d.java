...
public class Coords3d extends Basic3dFunc<Coords3d> {
    ...
    /**
     * Can be used to move the object that tiny bit that will solve the
     * different annoying manifold issues and CSG rendering problems.
     */
    ...
    /**
     * Creates a coordinate based on the given byte array. This is the inverse
     * operation of the {@link Abstract3d#toByteArray()} method.
     * 
     * @param bb
     *            the byte buffer to be read
    ...
    public static Coords3d midPoint(Coords3d a, Coords3d b) {
        return new Coords3d((a.x + b.x) / 2.0, (a.y + b.y) / 2.0,
                (a.z + b.z) / 2.0);
    }
    ...
    /**
     * Moving this coordinate by the given vector, but this object will be
     * unchanged and this method will create a new object with the new
     * coordinates.
     * 
     * @param move
     *            the vector used by the move
    ...
    /**
     * Rotating this coordinate by the given angle, but this object will be
     * unchanged and this method will create a new object with the new
     * coordinates.
     * 
     * @param angles
     *            the angles used by the rotation
     * @return a new coordinate instance which points to the new location
     */
    public Coords3d rotate(Angles3d angles) {
        ...
        if (!angles.isXZero()) {
            ...
            result = new Coords3d(result.x, xAngle.cos() * result.y
                    - xAngle.sin() * result.z, xAngle.sin() * result.y
                    + xAngle.cos() * result.z);
        }
        if (!angles.isYZero()) {
            Angle yAngle = angles.getYAngle();
            result = new Coords3d(yAngle.cos() * result.x + yAngle.sin()
                    * result.z, result.y, -yAngle.sin() * result.x
                    + yAngle.cos() * result.z);
        }
        if (!angles.isZZero()) {
            Angle zAngle = angles.getZAngle();
            result = new Coords3d(zAngle.cos() * result.x - zAngle.sin()
                    * result.y, zAngle.sin() * result.x + zAngle.cos()
                    * result.y, result.z);
        }
        return result;
    }
    ...
    /**
     * Return the inverse of this coordinate which means every coordinates will
     * be negated.
     * 
    ...
    public Coords3d cross(Coords3d a) {
        return new Coords3d(this.y * a.z - this.z * a.y, this.z * a.x - this.x
     ...
     *            interpolation value
     *
     * @return copy of this vector if {@code t = 0}; copy of {@code a} if
     *         {@code t = 1}; the point midway between this and the specified
     *         vector if {@code t = 0.5}
     */
