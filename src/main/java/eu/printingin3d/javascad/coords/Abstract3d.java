...
/**
 * The base class of all 3D related coordinate classes such as dimensions,
...
public abstract class Abstract3d {
    ...
    public final boolean isIdent() {
        return DoubleUtils.equalsEps(x, 1.0) && DoubleUtils.equalsEps(y, 1.0)
    ...
    public String format(OutputFormat outputFormat) {
        return new StringBuilder().append(outputFormat.getPreText())
                .append(DoubleUtils.formatDouble(x))
                .append(outputFormat.getSeparator())
                .append(DoubleUtils.formatDouble(y))
                .append(outputFormat.getSeparator())
                .append(DoubleUtils.formatDouble(z))
                .append(outputFormat.getPostText()).toString();
    }

    /**
     * Returns the byte array representation of this object used by the binary
     * STL export function. There will be 3x4 bytes to be written to the output
     * buffer.
     * 
     * @param byteBuffer
     *            the byte buffer where the coordinates will be written.
     */
    ...
    /**
     * Checks a much closer equal relation than the equals method does. The
     * difference could be much smaller this way.
     * 
     * @param value1
     *            the first value to be checked
     * @param value2
     *            the second value to be checked
     * @return true if and only if the two coordinate is very close to each
     *         other
     */
