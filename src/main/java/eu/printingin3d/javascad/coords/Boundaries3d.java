...
/**
 * Represents three intervals interpreting as an including cuboid which could
 * contains the actual 3D object or objects.
 *
...
public class Boundaries3d {
     ...
     */
    public static final Boundaries3d EMPTY = new Boundaries3d(Boundary.EMPTY,
            Boundary.EMPTY, Boundary.EMPTY);
    ...
    /**
     * Calculate the including cuboid so the result of this method is an
     * interval representing a cuboid that all of the parameter cuboids are
     * inside in this cuboid.
     * 
     * @param boundaries
     *            the intervals considered during the calculation
    ...
    public static Boundaries3d combine(Collection<Boundaries3d> boundaries) {
        ...
        return new Boundaries3d(Boundary.combine(collectX),
                Boundary.combine(collectY), Boundary.combine(collectZ));
    }

    /**
     * Calculate the inner cuboid so the result of this method is an interval
     * representing a cuboid that is inside all of the parameter cuboids.
     * 
     * @param boundaries
     *            the intervals considered during the calculation
     * @return a new object representing the including cuboid
     * @throws IllegalValueException
     *             if the given boundaries have no common part
     */
    public static Boundaries3d intersect(Collection<Boundaries3d> boundaries)
            throws IllegalValueException {
        List<Boundary> collectX = new ArrayList<>();
        List<Boundary> collectY = new ArrayList<>();
        List<Boundary> collectZ = new ArrayList<>();
        ...
        return new Boundaries3d(Boundary.intersect(collectX),
                Boundary.intersect(collectY), Boundary.intersect(collectZ));
    }
     ...
     * 
     * @param x
     *            the boundary on the X plane
     * @param y
     *            the boundary on the Y plane
     * @param z
     *            the boundary on the Z plane
     */
    ...
    /**
     * Returns with the size of this boundaries. The result will be a
     * {@link Dims3d} object which means it can be used to create a cube with
     * the same size as this model.
     * 
     ...
     * 
     * @param delta
     *            the value used by the move
     * @return a new object with the calculated new boundaries
     */
    public Boundaries3d move(Coords3d delta) {
        return new Boundaries3d(x.add(delta.getX()), y.add(delta.getY()),
                z.add(delta.getZ()));
    }

    /**
     * Adds the interval with the given delta - this operation always increase
     * the size of the boundaries.
     * 
     * @param delta
     *            the value used by the add operation
     * @return a new object with the calculated new boundaries
     */
     ...
     * 
     * @param delta
     *            the value used by the scale
     * @return a new object with the calculated new boundaries
     */
    public Boundaries3d scale(Coords3d delta) {
        return new Boundaries3d(x.scale(delta.getX()), y.scale(delta.getY()),
                z.scale(delta.getZ()));
    }

    /**
     * Rotates the interval with the given angle. The rotation takes the cuboid
     * represented by this object, rotates that and then returns with the
     * cuboids parameter which can contain this result.
     * 
     * @param angle
     *            the value used by the rotation
     * @return a new object with the calculated new boundaries
     */
    public Boundaries3d rotate(Angles3d angle) {
        Coords3d minCorner = getMinCorner();
        Coords3d maxCorner = getMaxCorner();
        return new Boundaries3d(minCorner.rotate(angle),
                maxCorner.rotate(angle));
    }
     ...
     * 
     * @param side
     *            the side we want to get
     * @return the point we asked for
     */
    public Coords3d getAlignedValue(Side side) {
        return new Coords3d(side.getAlignX().getAlignedValue(x), side
                .getAlignY().getAlignedValue(y), side.getAlignZ()
                .getAlignedValue(z));
    }

    /**
     * Returns true if and only if this boundary3d is inside of the given
     * boundary3d. It means it gives true if for each dimension the given
     * boundary's minimum value is not greater then this boundary's minimum
     * value and the given boundary's maximum value is not less then this
    ...
    public boolean isInsideOf(Boundaries3d boundaries) {
        return x.isInsideOf(boundaries.getX())
                & y.isInsideOf(boundaries.getY())
                & z.isInsideOf(boundaries.getZ());
    }
